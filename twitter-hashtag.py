...
def mainloop():
    ...
    while True:
        ...
        try:
            scrollphathd.clear()
            status = q.get(False) #get the tweet
            scrollphathd.write_string(status,font=font5x7, brightness=0.1) # write to the display
            ...
            while status_length > 0:
                ...
                status_length -= 1
                time.sleep(0.02) #scroll speed of tweet

            scrollphathd.clear() #clear out the tweet
            scrollphathd.show()
            time.sleep(20)
            ...
            time.sleep(5) #if the queue is empty, wait a bit
