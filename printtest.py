...
def gethashtags():
    ...
    lines=[]
    for x in range(numResults): #get just the limit keep 
        lines.append(trends[x]['name'] +'-'+str(trends[x]['tweet_volume']))
    print('getting api hashtags')
...
def display(lines):
    ...
    while True:  
        scrolldisplayfake(lines)

        if time.time() > timeout:
            gethashtags()
            break

def scrolldisplayfake(lines):
    #emulating the print cycle 
        for line in lines: 
            print(line) 
            time.sleep(2)
        print("")

def scrolldisplay(lines):
